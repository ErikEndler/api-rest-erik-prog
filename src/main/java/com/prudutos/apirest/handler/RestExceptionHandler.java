...
public class RestExceptionHandler  {	
	...
	public ResponseEntity<?> handlerResourceNotFoundException(ResourceNotFoundException rfnException) {
		ResourceNotFoundDetails rnfDetais = ResourceNotFoundDetails.builder().withTimestamp(new Date().getTime())
	...
	public ResponseEntity<?> handlerMethodArgumentNotValidException(MethodArgumentNotValidException mavnException) {
		...
		ValidationErrorDetails rnfDetais = ValidationErrorDetails.Builder.newBuilder().timestamp(new Date().getTime())
				.title("Field Validation Error")
				.detail("Field Validation Error")
				.status(HttpStatus.BAD_REQUEST.value())
				.developerMessage(mavnException.getClass()
